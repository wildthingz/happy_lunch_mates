...
pub struct HappyLunchMates<'hlm> {
    slack: &'hlm slack_driver::SlackDriver<'hlm>,
    n_groups: u8,
...
impl<'hlm> HappyLunchMates<'hlm> {
    pub fn new (slack_channel: &'hlm slack_driver::SlackDriver, n_groups: &u8, restaurant_names: &Vec<&'hlm str>) -> HappyLunchMates<'hlm> {
        ...
        if  (slack_channel.channel_members.len() as f32 / number_group as f32) < 1.3 {
            let new_n_group: f32 = slack_channel.channel_members.len() as f32 / 1.3;
    ...
    pub fn assign_to_group(&mut self){
        ...
        let n_group_members: u8 = self.slack.channel_members.len() as u8;
        ...
        let mut channel_members: Vec<(&str, &str)> = self.slack.channel_members.clone();
